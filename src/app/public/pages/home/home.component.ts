...
import { HomeTranslations } from '../../models/lang.interface';


interface HomeTranslationsData extends Array<HomeTranslations> {}
...
export class HomeComponent implements OnInit {
  ...
  constructor(private translateService: TranslateService) {
    this.translateService.get('home').subscribe((res: HomeTranslationsData) => {
      if (res && res[0]) {
        this.bannerTexts = res[0].full_banner_text;
        this.gridTexts = res[0].square_banner_text;
      } else {
        console.error('La respuesta del servicio de traducción no tiene la estructura esperada.');
      }
    });
  }
